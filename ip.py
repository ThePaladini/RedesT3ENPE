...
class IP:
    ...
    def __raw_recv(self, datagrama):
        ...
        if dst_addr == self.meu_endereco:
            ...
            if ttl>0:
                ...
                datagramaRemete= struct.pack('!BBHHHBBHII', 69, dscp | ecn, 48, identification, flags | frag_offset, 64, IPPROTO_ICMP, 0, int(ipaddress.ip_address(self.meu_endereco)),int(ipaddress.ip_address(src_addr)))
                aux = calc_checksum(datagrama)   
                datagramaRemete= struct.pack('!BBHHHBBHII', 69, dscp | ecn, 48, identification, flags | frag_offset, 64, IPPROTO_ICMP, aux, int(ipaddress.ip_address(self.meu_endereco)),int(ipaddress.ip_address(src_addr)))
                msgERRO = struct.pack('!BBHHH', 11, 0, 0, 0, 0)
                aux2 = calc_checksum(datagramaRemete + msgERRO + datagrama[:28])
    ...
    def _next_hop(self, dest_addr):
        ...
        if (len(arr)==1):                           
            ...
            final = (ipaddress.ip_network(self.tabela[arr[0]][0])).prefixlen
            ret = 0
            for i in range(len(arr)):
                aux = (ipaddress.ip_network(self.tabela[arr[i]][0])).prefixlen
                if aux > final:
                    final = aux
                    ret = i
